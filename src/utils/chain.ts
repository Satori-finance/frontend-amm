...
import { DEFAULT_NETWORK, NETWORK_ENV_CONFIG, NETWORK_ID_NAME, SUPPORTED_NETWORK_ID, TARGET_NETWORK_ID } from '@/const'
...
function redirectToChain(networkId: number) {
  ...
  if (TARGET_NETWORK_ID !== networkId && TARGET_NETWORK_ID !== config.CHAIN_ID) {
    // if no existed config, don't reload
